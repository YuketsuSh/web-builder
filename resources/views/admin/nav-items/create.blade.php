    ...
    <div class="flex">
        ...
        <div class="flex-1 p-6">
            <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
                <h2 class="text-2xl font-semibold text-gray-800 dark:text-gray-200">{{ __('Create Nav Item') }}</h2>

                <form action="{{ route('admin.nav-items.store') }}" method="POST" class="mt-4">
                    @csrf

                    <div class="mb-4">
                        <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Title') }}</label>
                        <input type="text" name="title" id="title" class="block w-full mt-1 border-gray-300 dark:border-gray-600 rounded-md shadow-sm" required>
                    </div>

                    <div class="mb-4">
                        <label for="url" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('URL') }}</label>
                        <input type="text" name="url" id="url" class="block w-full mt-1 border-gray-300 dark:border-gray-600 rounded-md shadow-sm" required>
                    </div>
